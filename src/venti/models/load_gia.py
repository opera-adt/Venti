...
def load_ice6g_model(file_path: str) -> gpd.GeoDataFrame:
    ...
    if file_path is None:
        raise ValueError(
            "ice6g_path must be provided when loading ICE6G-D model"
        )
    ...
    ice6_df.loc[ice6_df.Lon > 180, 'Lon'] = (
        ice6_df.loc[ice6_df.Lon > 180, 'Lon'] - 360
    )
    ...
    ice6_gdf = ice6_gdf.rename(columns={
        'Drad_250': 'vlm_rate',
        'Lon': 'lon',
        'Lat': 'lat'
    })
...
def load_caron_model(file_path: str) -> gpd.GeoDataFrame:
    ...
    if file_path is None:
        raise ValueError(
            "caron_path must be provided when loading Caron model"
        )
    
    # Define column names
    names = [
        'lat', 'lon', 'vlm_rate', 'vlm_std',
    ...
    # Shift longitude from 0-360 to -180-180
    gia_df.loc[gia_df.lon > 180, 'lon'] = (
        gia_df.loc[gia_df.lon > 180, 'lon'] - 360
    )
...
def clip_gia_df(gdf: gpd.GeoDataFrame,
                aoi: gpd.GeoDataFrame,
                buffer_distance: float = DEFAULT_BUFFER_DISTANCE) -> gpd.GeoDataFrame:
    """
...
def rasterize_gdf(gdf: gpd.GeoDataFrame,
                  value_column: str) -> Tuple[np.array, dict]:
    """
