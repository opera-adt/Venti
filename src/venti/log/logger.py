...
from .config import LoggerConfig, StructuredFormatter, LogFormat, LogLevel
from .filters import PerformanceFilter


class LoggerManager:
    """
    Centralized logger management with support for multiple output formats
    ...
    def configure_from_dict(cls, config_dict: Dict[str, Any]) -> logging.Logger:
        ...
        config = LoggerConfig(
            ...
            enable_performance_tracking=config_dict.get('enable_performance_tracking', False),
    ...
    def configure_from_env(cls, 
                          name: str = "app",
                          env_prefix: str = "LOG") -> logging.Logger:
        """
        ...
        config = LoggerConfig(
            ...
            log_dir=os.getenv(f"{env_prefix}_DIR", "./logs"),
            console_format=LogFormat(os.getenv(f"{env_prefix}_CONSOLE_FORMAT", "console")),
            file_format=LogFormat(os.getenv(f"{env_prefix}_FILE_FORMAT", "file")),
            enable_file_logging=os.getenv(f"{env_prefix}_ENABLE_FILE", "true").lower() == "true",
            enable_json_logging=os.getenv(f"{env_prefix}_ENABLE_JSON", "false").lower() == "true",
            max_file_size=int(os.getenv(f"{env_prefix}_MAX_SIZE", "10485760")),
            backup_count=int(os.getenv(f"{env_prefix}_BACKUP_COUNT", "5")),
            enable_performance_tracking=os.getenv(f"{env_prefix}_PERFORMANCE", "false").lower() == "true"
        )
        return cls.get_logger(name, config)
...
def configure_logging_from_file(config_file: Union[str, Path]) -> logging.Logger:
    """
